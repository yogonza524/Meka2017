...
public class UtilKmeans {
    ...
    private static Color[] colores = {Color.RED, Color.BLUE, Color.GREENYELLOW, Color.AQUAMARINE, Color.ORANGE, Color.DARKMAGENTA};
    private static String[] colorName = {"Rojo", "Azul", "Verde", "Celeste", "Naranja", "Violeta"};
    ...
    public static String resultadoFinal(KMeansResultado resultado){
        ...
        for (Cluster cluster : resultado.getClusters()) {
            result = result + "--Elementos del Cluster " + i + " ("+colorName[i]+") --\n";
    ...
    public static Plot addClustGrafic2(ArrayList<Double> puntos, Axes axes, int color){
        double ancho = 0.001, diferencia = 0.00051;
        Plot puntoG;
        
        double y = (double) puntos.get(1);
        puntoG = new Plot(
            x -> y ,
            puntos.get(0)-diferencia, puntos.get(0)+diferencia, ancho,
            axes, 7.0, colores[color]
        );

        return puntoG;
    }
    
    public static Plot addHistClustGrafic(Punto puntos, Axes axes){
        double ancho = 0.001, diferencia = 0.00051;
        Plot puntoG;
    ...
    public static ArrayList<Double> getCoordenadasCluster2(Cluster cluster) {
        Punto puntoCluster = cluster.getCentroide();
        ArrayList<Double> coordenadas = new ArrayList<>();
        Float[] data = puntoCluster.getData();
        for (int j = 0; j < 2; j++) {
            coordenadas.add(data[j].doubleValue());
    ...
    public static Plot graficarLineaHistCluster(Punto punto2, Punto punto1, Axes axes, int i) {
         Plot puntoG;
        double largo = 0.01;
        double ancho = 1.5;
        float aux = punto2.get(0);
        if (Math.abs(punto1.get(0)-punto2.get(0))<largo) {
            aux = (float) (aux + largo);
        }
        final float x0 = aux;
        if (punto1.get(0)< punto2.get(0)) {
            puntoG = new Plot(
            x -> ((punto2.get(1)-punto1.get(1))/(x0-punto1.get(0))*(x-punto1.get(0)))+punto1.get(1),
            punto1.get(0), x0, largo,
            axes, ancho, Color.YELLOW
        );
        }else{
            puntoG = new Plot(
                x -> ((punto1.get(1)-punto2.get(1))/(punto1.get(0)-x0)*(x-x0))+punto2.get(1),
                x0, punto1.get(0), largo,
                axes, ancho, Color.YELLOW
            );
        }
        return puntoG;
