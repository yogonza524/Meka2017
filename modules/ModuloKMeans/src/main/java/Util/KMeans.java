...
//        ArrayList<Cluster> historialCluster = new ArrayList<>();
        ArrayList<Cluster> clusters = inicializarCentroides(puntos, k, tipoInicio); //Creo los K primeros centroides
        ArrayList<Punto> centroideClusterN = new ArrayList<>();
	...
	while (!finalizo(clusters)) {
	    ...
	    recalcularCentroides(clusters);
            for (int i = 0; i < clusters.size(); i++) {
                centroideClusterN.add(clusters.get(i).getCentroide());
            }
            cantIteraciones++;
	}
...
//        resultKMeans.setHistorialCluster(historialCluster);
        resultKMeans.setClusters(clusters);
        resultKMeans.setCentroideClusterN(centroideClusterN);
        resultKMeans.setCantIteraciones(cantIteraciones);
        return resultKMeans;
    }
